...
class DatasetGenerator:
    def __init__(
        ...
        """
        :param input_ids: torch Tensor with input_ids
        :param attention_mask: torch Tensor with attention_mask
        :param token_type_ids: torch Tensor with token_type_ids
        :param labels: torch Tensor with labels
        """
        # The data tensors
    ...
    def read_from_directory(self, vectors_dir: str = 'data/vectors'):
        """
        Reads self.input_ids, self.attention_mask, self.token_type_ids
         and self.labels from directory with files
        :param vectors_dir: path to directory with vectors
            written by write_to_files
        """
        if not isdir(vectors_dir):
            raise FileNotFoundError(
                    f"Could not find directory at {vectors_dir}."
                )

        if not all([isfile(join(vectors_dir, f)) for f in self.file_names]):
            raise FileNotFoundError(
                    f"Could not find all files in directory at {vectors_dir}."
                    f" Try function read_vectors_from_file."
                )
    ...
    def read_from_files(
        ...
        """
        Reads self.input_ids, self.attention_mask, self.token_type_ids
         and self.labels from .pt files
        :param f_input_ids: path to .pt (pytorch tensor) file with input_ids
        :param f_attention_mask: path to .pt (pytorch tensor)
            file with attention_mask
        :param f_token_type_ids: path to .pt (pytorch tensor)
            file with token_type_ids
        :param f_labels: path to .pt (pytorch tensor) file with labels
        """
    ...
    def write_to_files(self, vectors_dir: str = 'data/vectors'):
        """
        Writes self.input_ids, self.attention_mask, self.token_type_ids
         and self.labels to .pt files
        :params vectors_dir: path to destination directory
        """
        if not isdir(vectors_dir):
    ...
    def write_balanced_dataset_to_files(
                self,
                dataset_dir: str = 'data/balanced_dataset'
            ):
        """
        Write self.balanced_dataset_to_doc, self.balanced_dataset_to_entity
         and self.balanced_dataset to directory
        :params vectors_dir: path to destination directory
        """
        if not self.balanced_dataset:
            raise ValueError(
                    "Balanced dataset not initialized. "
                    "Try function get_balanced_dataset."
                )
    ...
    def read_balanced_dataset(
                self,
                dataset_dir: str = 'data/balanced_dataset'
            ):
        """
        Read self.balanced_dataset_to_doc, self.balanced_dataset_to_entity
         and self.balanced_dataset tensors from directory
        :param dataset_dir: directory with all files
        """
        if not isdir(dataset_dir):
            raise FileNotFoundError(
                    f"Could not find directory at {dataset_dir}."
                )
        ...
        if not all([isfile(join(dataset_dir, f)) for f in self.file_names]):
            raise FileNotFoundError(
                    f"Could not find all files in directory at {dataset_dir}."
                    f" Try function read_vectors_from_file."
                )
        ...
        self.balanced_dataset = TensorDataset(input_ids, attention_mask,
                                              token_type_ids, labels)

    def get_tensor_dataset(self) -> TensorDataset:
        """
        Returns self.dataset, if necessary, initializing from self.input_ids,
         self.attention_mask, self.token_type_ids, and self.labels
        :returns: self.dataset, a TensorDataset object
        """
        if not self.dataset:
    ...
    def get_dataset_to_x(
                self,
                docs_entities: List = None
        ...
        :param docs_entities: list with all document - mention - candidate info
        :returns: three lists of the length of the dataset with
            doc indices, in-doc mention indices, and candidate IDs
        """
        if not (self.dataset_to_doc
                and self.dataset_to_entity
                and self.dataset_to_candidate
                ):
            self.dataset_to_doc = []
            self.dataset_to_entity = []
            self.dataset_to_candidate = []
            ...
            for i_doc, doc_entities in enumerate(docs_entities):
                ...
                for i_entity, entity_info in enumerate(doc_entities):
                    ...
                    if entity_info['GroundTruth'] != 'B' \
                            and entity_info['Candidates']:
                        # All these candidates belong to current doc
                        self.dataset_to_doc.extend(
                                [i_doc] * len(entity_info['Candidates'])
                            )
                        self.dataset_to_entity.extend(
                                [i_entity] * len(entity_info['Candidates'])
                            )
                        self.dataset_to_candidate.extend(
                                entity_info['Candidates']
                            )
        return self.dataset_to_doc, \
            self.dataset_to_entity, \
            self.dataset_to_candidate
    ...
    def get_balanced_dataset(
        ...
        :param n_neg: the number of included negative samples for each entity.
        ...
        for i_doc, doc_entities in enumerate(docs_entities):
            ...
            for i_entity, entity_info in enumerate(doc_entities):
                ...
                if entity_info['GroundTruth'] != 'B' \
                    ...
                    self.dataset_to_candidate.extend(
                            entity_info['GroundTruth']
                        )

                    # Add any positive datapoint
                    # (i.e. where candidate is ground truth)
                    if entity_info['GroundTruth'] in entity_info['Candidates']:
                        local_gt_idx = entity_info['Candidates'].index(
                                entity_info['GroundTruth']
                            )
                        gt_idx = full_dataset_idx + local_gt_idx

                        # Keep track of document and entity index of datapoint
                        self.balanced_dataset_to_doc.append(
                                i_doc
                            )
                        self.balanced_dataset_to_entity.append(
                                i_entity
                            )
                        self.balanced_dataset_to_candidate.append(
                                entity_info['GroundTruth']
                            )
                    ...
                    neg_cands = [
                            c for c in entity_info['Candidates']
                            if c != entity_info['GroundTruth']
                        ]
                    ...
                    for random_cand in random_cands:
                        ...
                        local_cand_idx = \
                            entity_info['Candidates'].index(random_cand)
    ...
    def get_split_dataset(
        ...
        """
        Splits the dataset in given ratios to
        ...
        The datasets must be initialized.

        :param split_ratios: the ratios of train, validation, and test
            respectively, as a list of three ratio values
        :param dataset: 'full' or 'balanced' respectively for the full or
            the balanced dataset
        :param docs_entities: if not 'balanced', this is used to
            generate dataset_to_doc
        ...
        print(" Dataset ID ranges:")
        print(f"--      Train: [{1:>4}, {n_train:>4}]")
        print(f"-- Validation: [{n_train+1:>4}, {n_train + n_val:>4}]")
        print(f"--       Test: [{n_train + n_val + 1:>4}, "
              f"{n_train+n_val+n_test+1:>4}] (Tot: {n_docs+1:>4})\n")
        ...
        print(f"({train_indices[0]: >6}, {train_indices[-1]: >6})   "
              f"training set slice, "
              f"{(train_indices[-1] - train_indices[0]) / n_train: >6.1f} "
              f"candidates on average per document")
        print(f"({val_indices[0]: >6}, {val_indices[-1]: >6}) "
              f"validation set slice, "
              f"{(val_indices[-1] - val_indices[0]) / n_val: >6.1f} "
              f"candidates on average per document")
        print(f"({test_indices[0]: >6}, {test_indices[-1]: >6})       "
              f"test set slice, "
              f"{(test_indices[-1] - test_indices[0]) / n_test: >6.1f} "
              f"candidates on average per document")
    ...
    def get_data_loaders(self, batch_size: int = 32):
        """
        Makes a DataLoader for each subset from get_split_dataset
        ...
        if not (self.train_dataset and self.val_dataset and self.test_dataset):
            print("The split datasets have not been initialized."
                  "Please try function get_split_dataset first.")
            return
    ...
    def print_token_sequence_stats(self):
        """
        Prints metrics of the data vectors,
         specifically mention context sequence length,
         entity context sequence length and total sequence length
        """
        ...
        print(f"{sum(n_tots) / len(n_tots) :.1f} "
              f"average number of tokens")
        print(f"{sum(n_left_tokens) / len(n_left_tokens) :.1f} "
              f"average tokens in entity sequence.")
        print(f"Min: {min(n_left_tokens)}, max: {max(n_left_tokens)}")
        print(f"{sum(n_right_tokens) / len(n_right_tokens) :.1f} "
              f"average tokens in candidate sequence.")
    ...
    def get_dataset_balance_info(self) -> Tuple[int, int]:
        ...
        print(f"Negative labels: {neg:,}, positive: {pos:,}, "
              f"negative/positive: {neg / pos:.3f}")
        return neg, pos
